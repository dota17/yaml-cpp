...
TEST_F(HandlerSpecTest, Ex2_19_Integers) {
  ...
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:int", 0, "12345"));
  EXPECT_CALL(handler, OnScalar(_, "?", 0, "decimal"));
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:int", 0, "+12345"));
  EXPECT_CALL(handler, OnScalar(_, "?", 0, "octal"));
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:int", 0, "0o14"));
  EXPECT_CALL(handler, OnScalar(_, "?", 0, "hexadecimal"));
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:int", 0, "0xC"));
  EXPECT_CALL(handler, OnMapEnd());
  EXPECT_CALL(handler, OnDocumentEnd());
  Parse(ex2_19);
}
...
TEST_F(HandlerSpecTest, Ex2_20_FloatingPoint) {
  ...
  EXPECT_CALL(handler, OnMapStart(_, "?", 0, EmitterStyle::Block));
  EXPECT_CALL(handler, OnScalar(_, "?", 0, "canonical"));
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:float", 0, "1.23015e+3"));
  EXPECT_CALL(handler, OnScalar(_, "?", 0, "exponential"));
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:float", 0, "12.3015e+02"));
  EXPECT_CALL(handler, OnScalar(_, "?", 0, "fixed"));
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:float", 0, "1230.15"));
...
TEST_F(HandlerSpecTest, Ex2_21_Miscellaneous) {
  ...
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:str", 0, "012345"));
  EXPECT_CALL(handler, OnMapEnd());
  EXPECT_CALL(handler, OnDocumentEnd());
  Parse(ex2_21);
}
...
TEST_F(HandlerSpecTest, Ex2_22_Timestamps) {
  ...
  EXPECT_CALL(handler, OnMapStart(_, "?", 0, EmitterStyle::Block));
  EXPECT_CALL(handler, OnScalar(_, "?", 0, "canonical"));
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:timestamp", 0, "2001-12-15T02:59:43.1Z"));
  EXPECT_CALL(handler, OnScalar(_, "?", 0, "iso8601"));
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:timestamp", 0, "2001-12-14t21:59:43.10-05:00"));
  EXPECT_CALL(handler, OnScalar(_, "?", 0, "spaced"));
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:timestamp", 0, "2001-12-14 21:59:43.10 -5"));
  EXPECT_CALL(handler, OnScalar(_, "?", 0, "date"));
  EXPECT_CALL(handler, OnScalar(_, "tag:yaml.org,2002:timestamp", 0, "2002-12-14"));
  EXPECT_CALL(handler, OnMapEnd());
  EXPECT_CALL(handler, OnDocumentEnd());
  Parse(ex2_22);
}
