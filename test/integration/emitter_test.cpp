...
TEST_F(EmitterErrorTest, BadAnchorOnGroup) {
  out << BeginSeq << "test" << Anchor("foo") << EndSeq;
  ExpectEmitError(ErrorMsg::INVALID_ANCHOR);
}

TEST_F(EmitterErrorTest, BadTagOnGroup) {
  out << BeginSeq << "test" << VerbatimTag("!foo") << EndSeq;
  ExpectEmitError(ErrorMsg::INVALID_TAG);
}
